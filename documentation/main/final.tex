...
\begin{titlepage}
  ...
  \IfFileExists{vect.png}{\includegraphics[width=0.22\textwidth]{vect.png}\par}{}
  \vspace{0.15in}
  ...
  \FrameBoxCol{%
    \begin{minipage}{0.94\linewidth}
      ...
      \begin{tikzpicture}[
        ...
        \node[fnode, fill=blue!10] (init)   {Initialise peripherals (ADC, I\textsuperscript{2}C, UART) and load calibration};
        \node[fnode, fill=cyan!10, below=of init]   (sample){Sample sensors: Hall-effect channels + IMU axes};
        \node[fnode, fill=teal!10, below=of sample] (prep)  {Preprocess: filter, normalise, apply $\Delta_i$, build packet};

        \node[fdec, fill=yellow!20, below=of prep]   (wifiok){Wi-Fi\\available?};

        \node[fbranch, fill=green!15, below=10mm of wifiok, xshift=-0.22\linewidth] (tx)  {Transmit frame\\wait for ACK};
        \node[fbranch, fill=red!15, below=10mm of wifiok, xshift= 0.22\linewidth] (buf) {Buffer + retry\\with backoff};

        \coordinate (merge) at ($(tx.south)!0.5!(buf.south)+(0,-4mm)$);

        \node[fnode, fill=purple!10, below=7mm of merge] (next) {Continue sampling (next window)};

        \draw[farr,->](init)--(sample);
        \draw[farr,->](sample)--(prep);
        \draw[farr,->](prep)--(wifiok);

        \coordinate (drop) at ($(wifiok.south)+(0,-3mm)$);
        \draw[farr,->](wifiok.south)--(drop);

        \draw[farr,->](drop) -| node[pos=0.25,fill=white,inner sep=2pt]{Yes} (tx.north);
        \draw[farr,->](drop) -| node[pos=0.25,fill=white,inner sep=2pt]{No}  (buf.north);

        \draw[farr,->](tx.south)  -- ++(0,-4mm) -| (merge);
        \draw[farr,->](buf.south) -- ++(0,-4mm) -| (merge);

        \fill(merge) circle(0.9pt);
  ...
  \FrameBoxCol{%
    \begin{minipage}{0.94\linewidth}
      ...
      \begin{tikzpicture}[
        ...
        \node[fnode, fill=blue!10] (ing)  {Ingest packets $\rightarrow$ decode $\rightarrow$ store raw + CSV};

        \node[fnode, fill=cyan!10, below=of ing]  (lab)  
        {Label windows (participant / session metadata)};

        \node[fnode, fill=teal!10, below=of lab]  (prep) 
        {Preprocess: calibrate, normalise, filter, window $\rightarrow$ dataset version};

        \node[fnode, fill=orange!15, below=of prep] (train)
        {Train model $\rightarrow$ evaluate $\rightarrow$ export artefacts};

        \node[fnode, fill=green!15, below=of train] (infer)
        {Inference API $\rightarrow$ (label, confidence) $\rightarrow$ UI output};

        \draw[farr] (ing) -- (lab);
        \draw[farr] (lab) -- (prep);
        \draw[farr] (prep) -- (train);
        \draw[farr] (train) -- (infer);
      \end{tikzpicture}
      \vspace{6pt}
    \end{minipage}}%
...
\begin{figure*}[t]
  ...
  \IfFileExists{KICAD_SSGL_PDF.pdf}{%
    ...
    \IfFileExists{sslg_full_circuit.png}{%
      \FrameBoxText{\includegraphics[width=\linewidth,keepaspectratio]{sslg_full_circuit.png}}%
    }{%
